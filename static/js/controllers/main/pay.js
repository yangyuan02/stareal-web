...
stareal
    .controller("PayController", function ($rootScope,$scope, $stateParams, $api, $state, $alert, localStorageService) {
        ...
        $scope.max = localStorageService.get('max')
        ...
        $scope.param.beily = '';
        $scope.param.num = localStorageService.get('num');
        $scope.param.price = localStorageService.get('price');
        $scope.total = $scope.param.num*$scope.param.price;//总价
        $scope.csl = "0张优惠券可用";
        $scope.copon_price = "0.0";
        $scope.deliver_price = "0.0";
        $scope.total_price = "0.0";
        $scope.beily_price = '0.0元';
        $scope.coupon = {};

        $scope.live_mobile = $rootScope.tel;
        //获取我的贝里余额
        ...
        $scope.subNum = function () {
            ...
            $scope.param.num = $scope.param.num - 1;
            $scope.total = $scope.param.num*$scope.param.price;//总价
        };

        $scope.addNum = function () {
            if ($scope.param.num == $scope.max) {
                $scope.defstyle={
                    "color":"#ccc"
                }
        ...
        $scope.change = function () {
            if ($scope.coupon != undefined && $scope.coupon != null && $scope.coupon.id != "") {
                $scope.param.couponId = $scope.coupon.id;
            } else {
                $scope.param.couponId = "";
            }
        }
        ...
        $scope.SubmitOrder = function () {
            ...
            var _params = {ticketId: $scope.ticketId, ticketNum:$scope.param.num, deliverType: $scope.param.deliverType};
            // 校验
            // 快递
            ...
            // 校验
            // 现场取票
            if ($scope.param.deliverType == 2||$scope.param.deliverType == 3) {
                if (!$scope.live_name || !$scope.live_mobile) {
                    $alert.show('请填写取票信息!');
                    return;
                }
                if (!$scope.myreg.test($scope.live_mobile)) {
            ...
            $api.post("app/order/index/create", _params, true)
                .then(function (ret) {
                    $state.go('main.pay_store',{order_id: ret.data.orderId});
                }, function (err) {
                    $alert.show(err);
                })
        };
        ...
        $api.get("app/address/retrieve", {}, true)
            .then(function (ret) {
                ...
                if($scope.addresses.length>0){
                    $scope.addresses[lastChose].active = "active";
                    $scope.param.addressId = $scope.addresses[lastChose].id;
                }
            });
        /**
         * 地址选择
         */
        $scope.select = function (index, addressId) {
            //如果有选择过，就把选过的数据中active置空
            (lastChose !== -1) && ($scope.addresses[lastChose].active = "");
            $scope.addresses[index].active = "active";
            lastChose = index;
            $scope.param.addressId = addressId;
        }
